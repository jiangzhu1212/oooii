...
oWebmEncoder::oWebmEncoder( const oVideoEncodeCPU::DESC& _Desc, bool* _pSuccess) : Desc(_Desc)
{
	*_pSuccess = false;
	...
	if(Desc.Dimensions.x < 1 || Desc.Dimensions.x > 16382 || Desc.Dimensions.y < 1 || Desc.Dimensions.y > 16382)
	{
	...
	tracks.VideoTrack.Width = static_cast<unsigned short>(_Desc.Dimensions.x);
	tracks.VideoTrack.Height = static_cast<unsigned short>(_Desc.Dimensions.y);
	tracks.VideoTrack.Codec = Codec;
	tracks.Write(Header);

	FrameCount = 0;
	MaxPayloadSize = Desc.Dimensions.x*Desc.Dimensions.y;
