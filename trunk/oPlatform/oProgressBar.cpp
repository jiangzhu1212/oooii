	...
	, hMarqueeBar(nullptr)
{
	*_pSuccess = oDispatchQueueCreatePrivate("oWinProgressBar Message Thread", 2000, &MessageQueue);
	if (*_pSuccess)
	{
		MessageQueue->Dispatch(oBIND(&oWinProgressBar::Initialize, this, _pSuccess));
		MessageQueue->Flush();
		MessageQueue->Dispatch(oBIND(&oWinProgressBar::Run, this));
	}
}

oWinProgressBar::~oWinProgressBar()
{
	MessageQueue->Dispatch(oBIND(&oWinProgressBar::Deinitialize, this));
	oWinWake(hDialog);
	MessageQueue->Join();
}
...
{
	DescMutex.unlock();
	MessageQueue->Dispatch(oBIND(&oWinProgressBar::SetDesc, thread_cast<oWinProgressBar*>(this), thread_cast<DESC&>(PendingDesc)));
	oWinWake(hDialog);
}
