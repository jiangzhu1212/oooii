...
template<> const char* oAsString(const oKeyboard::KEY& _Key)
{
	...
	static_assert(oCOUNTOF(sKeyStrings) == oKeyboard::NUM_KEYS, "");
...
struct Keyboard_Impl : public oKeyboard
{
	...
	oDEFINE_TRIVIAL_QUERYINTERFACE(oKeyboard);
...
{
	if (!_WindowNativeHandle || !_ppKeyboard)
		return oErrorSetLast(oERROR_INVALID_PARAMETER);
	bool success = false;
	oCONSTRUCT(_ppKeyboard, Keyboard_Impl((HWND)_WindowNativeHandle, _ShortCircuitKeyEvents, &success));
	return success;
}
