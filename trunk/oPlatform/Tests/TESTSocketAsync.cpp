...
struct TESTSocketAsync : public oTest
{
	struct GenericCallback : public oSocketAsyncCallback
	{
		...
		void ProcessSocketSend(void*_pData, oSocket::size_t _SizeData, const oNetAddr& _Addr, interface oSocket* _pSocket) threadsafe override
		{ 
			SendCount.fetch_add(1);
		}
	...
	RESULT TestUDPServerASYNC(char* _StrStatus, size_t _SizeofStrStatus, unsigned short Port)
	{
		...
		char AddrStr[64];
		sprintf_s(AddrStr, "localhost:%u", Port);
	...
	RESULT TestTCPServerASYNC(char* _StrStatus, size_t _SizeofStrStatus)
	{
		...
		oSocketServer::DESC ServerDesc;
		ServerDesc.ListenPort = 4545;

		oTESTB(oSocketServerCreate("TestTCPServerASYNC", ServerDesc, &Server), oErrorGetLastString());
		...
		oSocket::DESC Desc;
		Socket->GetDesc(&Desc);
		if(oSocket::oMSG == Desc.Protocol)
			Receiver->InitiateReceive(Socket, true);
		else
