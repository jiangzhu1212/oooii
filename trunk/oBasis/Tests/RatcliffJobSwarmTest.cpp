...
public:
    ...
    mX1 = x1;
    mY1 = y1;
		_pDispatchQueue->Dispatch(oBIND(&MandelbrotJob::Run, this));
	}
...
bool RunDispatchQueueTest(const char* _Name, threadsafe oDispatchQueue* _pDispatchQueue)
{
	...
	oErrorSetLast(oERROR_NONE, DEBUG_DISCLAIMER "Ratcliff/Mandelbrot: %s (%s sched)", timeStr, scheduleLoopStr);
	return true;
}
...
bool RunParallelForTest(const char* _Name)
{
	...
	delete [] fractal;
	oErrorSetLast(oERROR_NONE, DEBUG_DISCLAIMER "Ratcliff/Mandelbrot: %s", timeStr);
	return true;
}
