	...
	:Desc(_Desc), File(NULL)
{
	*_pSuccess = false;
	...
	sampleDescriptionHeader.DataOffset = FTell();
	...
	Desc.Width = sampleDescription.Width;
	Desc.Height = sampleDescription.Height;
	...
	sampleToChunkEntryHeader.DataOffset = FTell();
	...
	timeToSampleEntryHeader.DataOffset = FTell();
	...
	__int64 FrameDataOffset;
	for(unsigned int i = 0;i < NumberOfFrames;++i)
	{
		if(Is64BitFile)
		{
			succeeded &= FSeek(FrameFileOffsets+i*sizeof(__int64));
			succeeded &= FRead(FrameDataOffset);
			FrameDataOffset = oByteSwap(FrameDataOffset);
		}
		else
		{
			succeeded &= FSeek(FrameFileOffsets+i*sizeof(unsigned int));
...
bool oMOVFileImpl::QueryInterface(const oGUID& _InterfaceID, threadsafe void** _ppInterface) threadsafe
{
...
void oMOVFileImpl::FindAtoms(std::vector<oAtomHeader> &_results,const std::vector<const char*> &_searchTokens,unsigned int _searchIndex)
{
	...
	while(ReadHeader(nextAtomHeader))
	{
		...
		FSeek(AtomStartOffset+nextAtomHeader.Size);

		AtomStartOffset = FTell();
	}
}
...
void oMOVFileImpl::FindAtoms(std::vector<oAtomHeader> &_results,const char *_searchToken,const oAtomHeader * const _startAtom /*= NULL*/)
{
	...
	if(_startAtom)
	{
		if(!FSeek(_startAtom->DataOffset))
		{
...
void oMOVFileImpl::MapFrameInternal(void** _ppFrameData, size_t* _szData, bool* _pValid, size_t *_decodedFrameNumber)
{
	...
	FSeek(FileOffsetTable[CurrentFrame]);
