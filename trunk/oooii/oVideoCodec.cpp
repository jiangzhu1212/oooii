...
const oGUID& oGetGUID(threadsafe const oVideoDecodeCPU* threadsafe const * )
{
...
bool oVideoDecodeCPU::Create( threadsafe oVideoContainer* _pContainer, threadsafe oVideoDecodeCPU** _ppFrameDecoder )
{
...
bool oVideoDecodeD3D10::Create( DESC _desc, threadsafe oVideoDecodeD3D10** _ppDecoder )
{
	...
	CPUDecoders.resize(_desc.pContainers.size());
	for (unsigned int i = 0;i < _desc.pContainers.size();++i)
	{
		if( !oVideoDecodeCPU::Create(_desc.pContainers[i], &(CPUDecoders[i]) ) )
...
bool oVideoStream::Create( const oVideoContainer::DESC& _Desc, threadsafe oVideoStream** _ppVideoStream )
{
	bool success = false;
	switch( _Desc.ContainerType )
	{
